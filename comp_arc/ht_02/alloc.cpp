...
int main(){
	...
	std::cout << "memoryBegin = " << memoryBegin << std::endl;
	std::cout << "MCB_SIZE = " << MCB_SIZE << std::endl;
		...
		*/
		...
		while(1){
			...
			if(str == "ALLOC"){
				...
				std::cout << "ALLOC " << newBlockSize << std::endl;
				...
				std::cout << "FREE " << address << std::endl;

				my_delete((void*)address);
			
				// printf("error: programm could not recognise command parameter: \"%d\"\n", address);
